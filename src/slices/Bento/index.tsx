  ...
  return (
    ...
    >
      <PrismicRichText
        ...
        components={{
          heading2: ({ children }) => (
            <h2 className="text-balance text-center text-5xl font-medium md:text-7xl max-w-3xl ">
              {children}
            </h2>
          ),
          em: ({ children }) => (
            <em className="bg-gradient-to-b from-yellow-100 to-yellow-300 bg-clip-text not-italic text-transparent">
              {children}
            </em>
          ),
        }}
      ...
      <div className="mt-16 grid max-w-4xl grid-rows-[auto-auto-auto] gap-8 md:grid-cols-3 md:gap-10">
        {slice.items.map((item) => (
          <div
            className={clsx("glass-container row-span-3 grid grid-rows-subgrid gap-4 rounded-lg bg-gradient-to-b from-gray-900 to-gray-950 p-4", item.wide ? "md:col-span-2" : "md:col-span-1")}
